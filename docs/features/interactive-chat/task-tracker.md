...
- **次の優先タスク**: Task 2.1 (ChatInterface実装)
...
- **推定時間**: 6時間
- **成果物**:
  - [ ] `src/ui/chat/chat-interface.ts` (新規)
- **完了条件**:
  - [ ] tmuxペインへの接続が確立する
  - [ ] 基本的な入出力が動作する
  - [ ] イベントが正しく発行される
...
- **推定時間**: 4時間
- **成果物**:
  - [ ] `src/ui/chat/input-handler.ts` (新規)
- **完了条件**:
  - [ ] キー入力が正しくキャプチャされる
  - [ ] 特殊キー（Enter、矢印キー等）が処理される
  - [ ] 入力バッファが適切に管理される
...
- **推定時間**: 4時間
- **成果物**:
  - [ ] `src/ui/chat/chat-renderer.ts` (新規)
- **完了条件**:
  - [ ] メッセージが正しく表示される
  - [ ] スクロールが適切に動作する
  - [ ] フォーマットが適用される
...
### 優先順位
1. Task 2.1: ChatInterface実装
2. Task 2.2: InputHandler実装
3. Task 2.3: ChatRenderer実装
